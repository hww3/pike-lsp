...
import { createMockConnection, createMockServices, createMockDocuments, makeCacheEntry, sym } from '../helpers/mock-services.js';
...
describe('Cross-File Type Hierarchy', () => {
    ...
    describe('Multi-level cross-file inheritance', () => {
        it('should traverse inheritance chain across multiple files (FAILING - not implemented)', async () => {
            ...
            assert.strictEqual(grandParent.uri, grandParentUri, 'GrandParent should be from grandparent.pike');
        });
    });
    ...
    describe('Cross-file circular inheritance detection', () => {
        it('should detect circular inheritance across files (FAILING - not implemented)', async () => {
            ...
            assert.strictEqual(circularDiag.severity, DiagnosticSeverity.Error, 'Circular inheritance should be error');
        });
    });
});
