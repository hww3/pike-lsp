...
int main() {
    ...
    return 0;
      ...
      assert.ok(
        ['function', 'method'].includes(helperFunc.kind),
        'Helper should be a function or method'
      );
    });
...
int main() {
    ...
    return c->add(1, 2);
      ...
      assert.ok(hasAddMethod, 'Should find add method (as child or separate symbol)');
    });
